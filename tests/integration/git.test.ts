...
async function initRepo(dir: string): Promise<void> {
  ...
  execSync('git config user.email "test@aco.dev"', { cwd: dir, stdio: 'pipe' })
  execSync('git config user.name "ACO Test"', { cwd: dir, stdio: 'pipe' })
}

async function writeAndCommit(dir: string, filename: string, content: string, message: string): Promise<string> {
...
describe('GitClient', () => {
  ...
  it('stageFile + commit creates a commit and returns a non-empty hash', async () => {
    await fs.writeFile(path.join(tmpDir, 'page.tsx'), 'initial content', 'utf-8')
    execSync('git add page.tsx', { cwd: tmpDir, stdio: 'pipe' })
    execSync('git commit -m "baseline"', { cwd: tmpDir, stdio: 'pipe' })

    // Make a change
    await fs.writeFile(path.join(tmpDir, 'page.tsx'), 'updated content', 'utf-8')
    const client = createGitClient(tmpDir)
  ...
  it('getLastCommitHash matches the hash returned by commit', async () => {
    ...
    expect(lastHash.startsWith(committedHash) || committedHash.startsWith(lastHash)).toBe(true)
  })
  ...
  it('readFileAtHead returns the committed content of a file', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, 'app.tsx'), 'modified but not committed', 'utf-8')
  ...
  it('readFileAtHead throws GitError for a non-existent file', async () => {
    ...
    const client = createGitClient(tmpDir)
    await expect(client.readFileAtHead('nonexistent.tsx')).rejects.toBeInstanceOf(GitError)
  })
  ...
  it('revertCommit creates a new revert commit', async () => {
    ...
    const changeHash = await writeAndCommit(tmpDir, 'page.tsx', 'v2', 'change to revert')
